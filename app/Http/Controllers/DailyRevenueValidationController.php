    ...
    try {
        ...
        * -----------------------------------------------------------------*/
        ...
        $lub = isset($catIds['Lubrifiant'])
            ? (clone $productBase)
                ->whereHas('productPackaging.product', fn ($q) =>
                    $q->where('category_id', $catIds['Lubrifiant']))
                ->sum('total_price')
            : 0;
        ...
        $pea = isset($catIds['Produits d\'Entretien Auto (PEA)'])
            ? (clone $productBase)
                ->whereHas('productPackaging.product', fn ($q) =>
                    $q->where('category_id', $catIds['Produits d\'Entretien Auto (PEA)']))
                ->sum('total_price')
            : 0;
        ...
        $gaz = isset($catIds['GAZ'])
            ? (clone $productBase)
                ->whereHas('productPackaging.product', fn ($q) =>
                    $q->where('category_id', $catIds['GAZ']))
                ->sum('total_price')
            : 0;
        ...
        $lampes = isset($catIds['Lampes'])
            ? (clone $productBase)
                ->whereHas('productPackaging.product', fn ($q) =>
                    $q->where('category_id', $catIds['Lampes']))
                ->sum('total_price')
            : 0;
